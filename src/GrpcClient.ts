...
export class GrpcClient extends PadLocalClientPlugin {
  ...
  private async _onServerMessage(serverMessage: ActionMessage): Promise<void> {
    ...
    log.debug(
      `[tid:${
        this.traceId
      }] receive event from server, seq:${seq} ack:${ack}, type:${serverMessage.getPayloadCase()}, payload:${Utils.stringifyPB(
        payload
      )}`
    );
  ...
  private _sendMessage<T extends Message>(payload: T, seq?: number, ack?: number) {
    if (this._status !== GrpcClient.Status.OK && this._status !== GrpcClient.Status.SERVER_COMPLETE) {
      throw new GrpcClient.SubRequestCancelError(
        this._status,
    ...
    log.debug(
      `tid:[${
        this.traceId
      }] send event to server, seq:${seq}, ack:${ack}, type: ${actionMessage.getPayloadCase()}, payload: ${Utils.stringifyPB(
        payload
      )}`
    );
