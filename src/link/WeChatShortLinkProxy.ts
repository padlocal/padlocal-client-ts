    ...
    async send(path: string, data: Bytes): Promise<Bytes> {
        try {
            ...
            if (!this.retryStrategy.canRetry()) {
                let message = `[tid:${this.traceId}] Fail to request short link for path:${path}, data: ${ByteUtils.bytesToHexString(data)}, after max retry:${this.retryStrategy.retryCount}`;
            ...
            const req = http.request(
                ...
                (res) => {
                    if (res.statusCode && res.statusCode >= 400) {
                        reject(new WeChatShortLinkProxy.HttpError(`http status error, status: ${res.statusCode}, message:${res.statusMessage}`));
                    }
