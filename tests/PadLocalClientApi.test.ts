...
describe("message", () => {
  describe("send", () => {
    ...
    const atUserList: Array<string> = config.get("test.message.send.atUserNameList");
  ...
  describe("get message payload", () => {
    ...
    test("get message with normal image", async () => {
      const message = Message.deserializeBinary(
        ByteUtils.hexStringToBytes(config.get("test.message.payload.normalImageMessage"))
      );

      const imageResult = await client.api.getMessageImage(message, ImageType.NORMAL);
      expect(imageResult.imageType == ImageType.NORMAL);
      ...
      const hdImageResult = await client.api.getMessageImage(message, ImageType.HD);
      expect(hdImageResult.imageType == ImageType.NORMAL);
      expect(hdImageResult.imageData.length).toBe(imageResult.imageData.length);
    });

    test("get message with hd image", async () => {
      const message = Message.deserializeBinary(
        ByteUtils.hexStringToBytes(config.get("test.message.payload.hdImageMessage"))
      );
      ...
      const hdImageResult = await client.api.getMessageImage(message, ImageType.HD);
      expect(hdImageResult.imageType == ImageType.HD);
    ...
    test("get message voice", async () => {
      const message = Message.deserializeBinary(
        ByteUtils.hexStringToBytes(config.get("test.message.payload.voiceMessage"))
      );
    ...
    test("get message videoThumb", async () => {
      const message = Message.deserializeBinary(
        ByteUtils.hexStringToBytes(config.get("test.message.payload.videoMessage"))
      );
    ...
    test("get message file", async () => {
      const message = Message.deserializeBinary(
        ByteUtils.hexStringToBytes(config.get("test.message.payload.fileMessage"))
      );
